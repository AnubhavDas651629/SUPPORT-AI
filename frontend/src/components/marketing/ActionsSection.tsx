      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)] lg:gap-12">
        <div className="overflow-hidden rounded-panel border border-line bg-surface">
          <div className="border-b border-line px-4 py-2.5 sm:px-5">
            <p className="font-code text-[10.5px] uppercase tracking-[0.1em] text-subtle">
            ...
            {CALLS.map((call) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={`rounded px-1.5 py-0.5 font-code text-[10.5px] font-medium ${
                      call.verb === "GET"
                  ...
                  <code className="font-code text-[12.5px] text-fg">{call.path}</code>
                </div>
                <p className="mt-1.5 text-[12.5px] text-muted">{call.purpose}</p>
                <p
                  className={`mt-1.5 font-code text-[11.5px] ${
        ...
        <div>
          ...
          <ul className="mt-5 flex flex-wrap gap-1.5">
            {EVENTS.map((event) => (
              <li
                key={event}
                className="rounded border border-line bg-surface-2 px-2 py-1 font-code text-[11.5px] text-muted"
              >
                {event}
              </li>
            ))}
          ...
          <div className="mt-6 overflow-hidden rounded-panel border border-line bg-bg-inset">
            <div className="border-b border-line px-3 py-2">
              <p className="font-code text-[10.5px] text-subtle">
                x-supportai-signature
              </p>
            </div>
            <pre className="overflow-x-auto px-3 py-3 font-code text-[11.5px] leading-relaxed text-muted">
{`sha256=9f2b1c...e04a
